...
std::shared_ptr<C_Boss> C_Game2::GetBoss()
{
	return m_enemymanager->GetBoss();
}

std::vector<std::shared_ptr<C_SubBoss>> C_Game2::GetSubBoss()
{
	return m_enemymanager->GetSubBoss();
}

void C_Game2::ResultPush()
{
...
void C_Game2::Init()
{
	...
	m_gamestartpattern = GameStartPattern2::GameStartDirection;
...
void C_Game2::Update()
{
	...
	switch (m_gamemode)
	{
	case GameMode2::Start:
		switch (m_gamestartpattern)
		{
		case GameStartPattern2::GameStartDirection:
			m_gamestartdirectiontime--;
			if (m_gamestartdirectiontime <= 0)
			{
				m_gamestartpattern = GameStartPattern2::StartDirection;
				m_startflg = true;
			}
			break;
		case GameStartPattern2::StartDirection:
			m_startscale *= 0.85f;
			if (m_startscale.x <= 1.0f)
			{
				...
				{
					m_startflg = false;
					m_gamestartpattern = GameStartPattern2::GameStart;
				}
			}

			break;
		case GameStartPattern2::GameStart:
			m_gamemode = GameMode2::Loop;
			break;
		default:
			break;
		}
